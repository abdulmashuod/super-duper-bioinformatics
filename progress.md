# Learning Progress Tracker

Track your daily progress through the 10-day bioinformatics fundamentals course.

## Daily Checklist

| Day | Topic | Article Read | Notes Taken | Concepts Listed | Resources Added | Status |
|-----|-------|:-------------:|:-----------:|:---------------:|:---------------:|:------:|
| 1 | Introduction to Bioinformatics | ✓| ✓ | ✓ | ✓ | ✓ |
| 2 | DNA/RNA Sequences & Structure | ✓ | ✓ | ✓ | ✓ | ✓ |
| 3 | NCBI Databases                | ✓  | ✓  | ✓  | ✓  | ✓  |
| 4 | Pairwise & Multiple Sequence Alignment | ☐ | ☐ | ☐ | ☐ | ⏳ |
| 5 | Protein Structure & Function | ☐ | ☐ | ☐ | ☐ | ⏳ |
| 6 | Gene Finding & Annotation | ☐ | ☐ | ☐ | ☐ | ⏳ |
| 7 | Phylogenetic Analysis | ☐ | ☐ | ☐ | ☐ | ⏳ |
| 8 | Bioinformatics Databases | ☐ | ☐ | ☐ | ☐ | ⏳ |
| 9 | Next-Generation Sequencing | ☐ | ☐ | ☐ | ☐ | ⏳ |
| 10 | Practical Tools & Pipelines | ☐ | ☐ | ☐ | ☐ | ⏳ |

**Legend**: ☐ = Not Started | ✓ = Completed | ⏳ = In Progress

---

## Daily Logs

### Day 1 - Introduction to Bioinformatics
- **Date Started**: 29-july-2026
- **Date Completed**: 29-july-2026
- **Time Spent**: 8Hours
- **Difficulty Level**: Easy 
- **Notes**: [notes-of-day-01](./Day-01-Introduction/notes.md)

### Day 2 - DNA/RNA Sequences & Structure
- **Date Started**: 30-07-2-206
- **Date Completed**: 30-07-2026
- **Time Spent**: 5 hours
- **Difficulty Level**: Easy 
- **Notes**: [notes-for-day-02](./Day-02-Biological_sequence&File-Formats/notes.md)

### Day 3 - Sequence Alignment Basics
- **Date Started**: 05-08-2026
- **Date Completed**:05-08-2026 
- **Time Spent**:2 hours 
- **Difficulty Level**: Easy  
- **Notes**:[notes-0f-day-03](./Day-03-NCBI-Databases/notes.md) 

### Day 4 - Multiple Sequence Alignment
- **Date Started**: 
- **Date Completed**: 
- **Time Spent**: 
- **Difficulty Level**: Easy / Medium / Hard
- **Notes**: 

### Day 5 - Protein Structure & Function
- **Date Started**: 
- **Date Completed**: 
- **Time Spent**: 
- **Difficulty Level**: Easy / Medium / Hard
- **Notes**: 

### Day 6 - Gene Finding & Annotation
- **Date Started**: 
- **Date Completed**: 
- **Time Spent**: 
- **Difficulty Level**: Easy / Medium / Hard
- **Notes**: 

### Day 7 - Phylogenetic Analysis
- **Date Started**: 
- **Date Completed**: 
- **Time Spent**: 
- **Difficulty Level**: Easy / Medium / Hard
- **Notes**: 

### Day 8 - Bioinformatics Databases
- **Date Started**: 
- **Date Completed**: 
- **Time Spent**: 
- **Difficulty Level**: Easy / Medium / Hard
- **Notes**: 

### Day 9 - Next-Generation Sequencing
- **Date Started**: 
- **Date Completed**: 
- **Time Spent**: 
- **Difficulty Level**: Easy / Medium / Hard
- **Notes**: 

### Day 10 - Practical Tools & Pipelines
- **Date Started**: 
- **Date Completed**: 
- **Time Spent**: 
- **Difficulty Level**: Easy / Medium / Hard
- **Notes**: 

---

## Summary Statistics

- **Total Days**: 10
- **Days Completed**: 3/10
- **Overall Progress**: 33%
- **Average Time per Day**: [Will calculate after completion]
- **Most Challenging Day**: TBD
- **Key Takeaway**: TBD

---

**Key Concepts Covered:**
## Day-01 Milestone
- [ ] What bioinformatics is (definition & scope)
- [ ] Interdisciplinary nature (4 pillars)
- [ ] Central Dogma: DNA → RNA → Protein
- [ ] Big data problem in biology
- [ ] Major databases (NCBI, GenBank, UniProt, Ensembl)
- [ ] Career pathways (4 main roles)
- [ ] Historical context (Dayhoff, Altschul)
- [ ] Real-world applications (cancer drugs, COVID-19)
## Day-02 Milestone
- [ ] Fasta Format
- [ ] Fastq Format
- [ ] SAM format
- [ ] BAM format 
- [ ] VCF format
- [ ] Additional formats
- [ ] Format Comparison 
- [ ] Tools for Format Conversion
## Day-03 Milestones
-[ ] Introduction to NCBI
-[ ] GenBank Databases
-[ ] PubMed Database
-[ ] Protein Database(NCBI databases)
-[ ] Interaction between Databases
- [ ] Practical Applications
-[ ] Best Practics

